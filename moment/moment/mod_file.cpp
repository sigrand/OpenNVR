...
bool
httpRequest (HTTPServerRequest &req, HTTPServerResponse &resp, void * _path_entry )
{
...
void momentFile_addPathEntry (PathEntry   * const path_entry)
{
    ...
    HttpReqHandler::addHandler(std::string(path_entry->prefix->cstr()), httpRequest, (void *) path_entry);
             ...
             ConstMemory   const prefix)
{
    ...
    momentFile_addPathEntry (path_entry);
}
...
//    ServerApp * const server_app = moment->getServerApp();
	...
	while (!modfile_section->iter_done (iter)) {
		...
		if (sect_entry->getName().len() == 0)
                    momentFile_addPathForSection (section);
            }
	}
    } while (0);
    ...
    if (!got_path) {
      // Default setup.
    momentFile_addPath ("/opt/moment/myplayer", "moment");
    momentFile_addPath ("/opt/moment/mychat", "mychat");
    }
}
