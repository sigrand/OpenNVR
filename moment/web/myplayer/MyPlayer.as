...
{
    private var aSlider:Slider;

    private var buffer_time : Number;
    private var autoplay : Boolean;

    private var start_time : int;
    private var end_time : int;
    private var seekUri : String;
    private var seekStream : String;
    private var seekTimer : Timer;

    private var accum : Number;
    ...
    private function setSourceSeeked (uri : String, stream_name_ : String, start_time_: int, end_time_: int) : void {
        trace ("--- setSourceSeeked(): " + uri + ", " + stream_name_ + "?start=" + start_time.toString());
        start_time = start_time_;
        accum = 0.0;
        end_time = end_time_;
        seekUri = uri;
        seekStream = stream_name_;
        setChannel (uri, stream_name_ + "?start=" + start_time.toString());
	...
	if (horizontal_mode || videoShouldBeHorizontal()) {
        ...
        aSlider.x = video.x + 10;
        aSlider.width = video.width - 20;

        aSlider.y = video.y + video.height + 20;
...
//	video.deblocking = 5;
    ...
    aSlider.addEventListener(SliderEvent.THUMB_RELEASE, thumbReleaseHandler); 
	...
	doResize ();
        ...
        seekTimer = new Timer(1000); // 1 second
        seekTimer.addEventListener(TimerEvent.TIMER, shiftSlider);
        seekTimer.start();
    ...
    private function thumbReleaseHandler(event:SliderEvent):void { 
            
        var seekPercent:int = event.value;
        var seekDuration:int = end_time - start_time;
        var seekStart:int = start_time + (seekDuration * seekPercent) / 100;
        accum = 0.0;

        setChannel (seekUri, seekStream + "?start=" + seekStart.toString());

    }

    private function shiftSlider(event:TimerEvent):void {
        
        var seekDuration:int = end_time - start_time;

        accum += 1.0 / Number(seekDuration);

        if(accum > 0.01)
        {
            var shiftVal:int = Math.floor(accum / 0.01);
            accum = accum - Number(shiftVal) * 0.01;

            if(aSlider.value < 100 && aSlider.enabled == true)
                aSlider.value += shiftVal;
        }
    }
}
