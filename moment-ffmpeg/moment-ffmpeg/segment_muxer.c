...
// MOMENT, NOTE: all changes in this file are made using following define:
#define MOMENT_CHANGE
...
#include "libavutil/timestamp.h"
    ...
    #include "ffmpeg_config.h" // config.h file from ffmpeg. It is the generated file. It is created on building of ffmpeg.
    #include <time.h>
    #include <errno.h>
    #ifdef HAVE_UNISTD_H
    #include <unistd.h>
...
#ifdef MOMENT_CHANGE    // MOMENT_CHANGE [
    av_log(s, AV_LOG_VERBOSE, "segment duration=%s, time_delta=%s,format=%s\n",
           av_ts2str(seg->time), av_ts2str(seg->time_delta), seg->format ? seg->format : "UNKNOWN OFMT");

    if(MakeFullPath(s->filename, seg->time / 1000000, oc->filename, sizeof(oc->filename)) < 0)
    {
        av_log(oc, AV_LOG_ERROR, "Invalid segment2 filename '%s'\n", s->filename);
...
#ifdef MOMENT_CHANGE
AVOutputFormat  ff_segment2_muxer   = {
    .name           = "segment2",
    .long_name      = NULL_IF_CONFIG_SMALL("segment2"),
...
#ifdef MOMENT_CHANGE
AVOutputFormat ff_stream_segment2_muxer = {
    .name           = "stream_segment2,ssegment2",
    .long_name      = NULL_IF_CONFIG_SMALL("streaming segment2 muxer"),
    .priv_data_size = sizeof(SegmentContext),
    .flags          = AVFMT_NOFILE,
    .write_header   = seg_write_header,
    .write_packet   = seg_write_packet,
