...
const int STATIOSIZE = 100; // I hope 100 would be enough
stat_io g_stat_io [100]; 

void flush_stat_io(stat_io * p_stat_io)
{
	p_stat_io->valid 	= 0;
	memset(p_stat_io->devname,0,256);
	p_stat_io->rrqm_s 	= 0.0;
	p_stat_io->wrqm_s 	= 0.0;
	p_stat_io->r_s 		= 0.0;
	p_stat_io->w_s 		= 0.0;
	p_stat_io->rkB_s 	= 0.0;
	p_stat_io->wkB_s 	= 0.0;
	p_stat_io->avgrq_sz = 0.0;
	p_stat_io->avgqu_sz = 0.0;
	p_stat_io->await 	= 0.0;
	p_stat_io->r_await 	= 0.0;
	p_stat_io->w_await 	= 0.0;
	p_stat_io->svctm 	= 0.0;
	p_stat_io->util 	= 0.0;
}

void avg_stat_io(stat_io * p_stat_io, int count)
{
	if(!count)
		return;
	p_stat_io->rrqm_s 		/= count;
	p_stat_io->wrqm_s 		/= count;
	p_stat_io->r_s 			/= count;
	p_stat_io->w_s 			/= count;
	p_stat_io->rkB_s 		/= count;
	p_stat_io->wkB_s 		/= count;
	p_stat_io->avgrq_sz 	/= count;
	p_stat_io->avgqu_sz 	/= count;
	p_stat_io->await 		/= count;
	p_stat_io->r_await 		/= count;
	p_stat_io->w_await 		/= count;
	p_stat_io->svctm 		/= count;
	p_stat_io->util 		/= count;
}

void assign_stat_io(stat_io * p_stat_io, stat_io * p_stat_io_added)
{
	p_stat_io->valid 	= p_stat_io_added->valid;
	memset(p_stat_io->devname, 0, 256);
	strcpy(p_stat_io->devname, p_stat_io_added->devname);
	p_stat_io->rrqm_s 	= p_stat_io_added->rrqm_s;
	p_stat_io->wrqm_s 	= p_stat_io_added->wrqm_s;
	p_stat_io->r_s 		= p_stat_io_added->r_s;
	p_stat_io->w_s 		= p_stat_io_added->w_s;
	p_stat_io->rkB_s 	= p_stat_io_added->rkB_s;
	p_stat_io->wkB_s 	= p_stat_io_added->wkB_s;
	p_stat_io->avgrq_sz = p_stat_io_added->avgrq_sz;
	p_stat_io->avgqu_sz = p_stat_io_added->avgqu_sz;
	p_stat_io->await 	= p_stat_io_added->await;
	p_stat_io->r_await 	= p_stat_io_added->r_await;
	p_stat_io->w_await 	= p_stat_io_added->w_await;
	p_stat_io->svctm 	= p_stat_io_added->svctm;
	p_stat_io->util 	= p_stat_io_added->util;
}
...
void write_ext_stat_to_struct(int curr, unsigned long long itv, int fctr,
		    struct io_hdr_stats *shi, struct io_stats *ioi,
		    struct io_stats *ioj, int i)
{
	char *devname = NULL;
	struct stats_disk sdc, sdp;
	struct ext_disk_stats xds;
	...
	g_stat_io[i].valid = 1;
	if(strlen(g_stat_io[i].devname) == 0)
    	strcpy(g_stat_io[i].devname, devname);
	g_stat_io[i].rrqm_s		+= S_VALUE(ioj->rd_merges, ioi->rd_merges, itv);
	g_stat_io[i].wrqm_s		+= S_VALUE(ioj->wr_merges, ioi->wr_merges, itv);
	g_stat_io[i].r_s 		+= S_VALUE(ioj->rd_ios, ioi->rd_ios, itv);
	g_stat_io[i].w_s 		+= S_VALUE(ioj->wr_ios, ioi->wr_ios, itv);
	g_stat_io[i].rkB_s 		+= ll_s_value(ioj->rd_sectors, ioi->rd_sectors, itv) / fctr;
	g_stat_io[i].wkB_s 		+= ll_s_value(ioj->wr_sectors, ioi->wr_sectors, itv) / fctr;
	g_stat_io[i].avgrq_sz 	+= xds.arqsz;
	g_stat_io[i].avgqu_sz 	+= S_VALUE(ioj->rq_ticks, ioi->rq_ticks, itv) / 1000.0;
	g_stat_io[i].await 		+= xds.await;
	g_stat_io[i].r_await 	+= r_await;
	g_stat_io[i].w_await 	+= w_await;
	g_stat_io[i].svctm 		+= xds.svctm;
	g_stat_io[i].util 		+= shi->used ? xds.util / 10.0 / (double) shi->used : xds.util / 10.0;
...
/*
	...
	shi = st_hdr_iodev;
	int iii = 0;
	for (iii = 0; iii < iodev_nr; iii++, shi++) 
	{
		printf("shi->name is %s\n", shi->name);
		if (shi->used) 
		{
			printf("shi is used\n");
			if(!DISPLAY_UNFILTERED(flags))
			{
				printf("shi is filtered\n");
			}
		}
	}
*/
		...
		for (i = 0; i < iodev_nr; i++, shi++) {
			if (shi->used) {
				...
				/* here filtered all unneeded devices*/
				...
				if (DISPLAY_EXTENDED(flags)) {
					...
					write_ext_stat_to_struct(curr, itv, fctr, shi, ioi, ioj, i);
					wasChange = 1;
				}
				else {
...
void init_iostat(/*int argc, char **argv*/)
{
	int it = 0;
	int opt = 1;
	...
	if(1) /* Select disk output unit (kB/s or blocks/s) */
		flags |= I_D_KILOBYTES;
	...
	*/
	...
	int ii=0;
	for(ii=0;ii< STATIOSIZE ;ii++)
	{
		flush_stat_io(& g_stat_io[ii]);
	}
...
void Get_stat_io(stat_io ** stat_io_out, int * stat_io_size_out)
{
	pthread_mutex_lock(&lock);
    ...
    for(i=0;i<STATIOSIZE;i++)
    {
        if(g_stat_io[i].valid)
        {
        	avg_stat_io(&g_stat_io[i], g_stat_io_count);
            size++;
        }
    }

    stat_io * iostat_str_arr_export_ = malloc(size * sizeof(stat_io));
    
    for(i=0;i<STATIOSIZE;i++)
    {
        if(g_stat_io[i].valid)
        {
        	assign_stat_io(&iostat_str_arr_export_[j], &g_stat_io[i]);
/*
		printf("g_stat_io[%d].devname = %s\n"
			"g_stat_io[%d].rrqm_s = %f\n"
			"g_stat_io[%d].wrqm_s = %f\n"
			"g_stat_io[%d].r_s = %f\n"
			"g_stat_io[%d].w_s = %f\n"
			"g_stat_io[%d].rkB_s = %f\n"
			"g_stat_io[%d].wkB_s = %f\n"
			"g_stat_io[%d].avgrq_sz = %f\n"
			"g_stat_io[%d].avgqu_sz = %f\n"
			"g_stat_io[%d].await = %f\n"
			"g_stat_io[%d].r_await = %f\n"
			"g_stat_io[%d].w_await = %f\n"
			"g_stat_io[%d].svctm = %f\n"
			"g_stat_io[%d].util = %f\n",
			i,g_stat_io[i].devname,
	       i,g_stat_io[i].rrqm_s,
	       i,g_stat_io[i].wrqm_s,
	       i,g_stat_io[i].r_s,
	       i,g_stat_io[i].w_s,
	       i,g_stat_io[i].rkB_s,
	       i,g_stat_io[i].wkB_s,
	       i,g_stat_io[i].avgrq_sz,
	       i,g_stat_io[i].avgqu_sz,
	       i,g_stat_io[i].await,
	       i,g_stat_io[i].r_await,
	       i,g_stat_io[i].w_await,
	       i,g_stat_io[i].svctm,
			i,g_stat_io[i].util);
	
	printf("iostat_str_arr_export_[%d].devname = %s\n"
			"iostat_str_arr_export_[%d].rrqm_s = %f\n"
			"iostat_str_arr_export_[%d].wrqm_s = %f\n"
			"iostat_str_arr_export_[%d].r_s = %f\n"
			"iostat_str_arr_export_[%d].w_s = %f\n"
			"iostat_str_arr_export_[%d].rkB_s = %f\n"
			"iostat_str_arr_export_[%d].wkB_s = %f\n"
			"iostat_str_arr_export_[%d].avgrq_sz = %f\n"
			"iostat_str_arr_export_[%d].avgqu_sz = %f\n"
			"iostat_str_arr_export_[%d].await = %f\n"
			"iostat_str_arr_export_[%d].r_await = %f\n"
			"iostat_str_arr_export_[%d].w_await = %f\n"
			"iostat_str_arr_export_[%d].svctm = %f\n"
			"iostat_str_arr_export_[%d].util = %f\n",
			j,iostat_str_arr_export_[j].devname,
	       j,iostat_str_arr_export_[j].rrqm_s,
	       j,iostat_str_arr_export_[j].wrqm_s,
	       j,iostat_str_arr_export_[j].r_s,
	       j,iostat_str_arr_export_[j].w_s,
	       j,iostat_str_arr_export_[j].rkB_s,
	       j,iostat_str_arr_export_[j].wkB_s,
	       j,iostat_str_arr_export_[j].avgrq_sz,
	       j,iostat_str_arr_export_[j].avgqu_sz,
	       j,iostat_str_arr_export_[j].await,
	       j,iostat_str_arr_export_[j].r_await,
	       j,iostat_str_arr_export_[j].w_await,
	       j,iostat_str_arr_export_[j].svctm,
			j,iostat_str_arr_export_[j].util);
*/	
    ...
    (*stat_io_out) = iostat_str_arr_export_;
    (*stat_io_size_out) = size;

	/* flush for next stat */
    for(i=0;i<STATIOSIZE;i++)
    {
    	flush_stat_io(&g_stat_io[i]);
    }
    g_stat_io_count = 0;
...
void Free_stat_io(stat_io ** stat_io_out)
{
	free(*stat_io_out);
	*stat_io_out = 0;
	stat_io_out = 0;
