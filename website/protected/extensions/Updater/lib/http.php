...
{ 
	...
	protected function fetch($url, $method, $maxredir = 10, $form = array(), $files = array())
	{
		...
		/* redirects: 302 */
		if ((isset($this->_head['Location']) || isset($this->_head['location'])) && $maxredir > 0) {
			$location = isset($this->_head['Location']) ? $this->_head['Location'] : $this->_head['location'];
			$maxredir--;
			$this->fetch($this->getAbsUrl($location, $url), 'GET', $maxredir);
		}
