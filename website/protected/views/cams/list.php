							...
							<tbody>
								...
								foreach ($myCams as $key => $cam) {
									...
									if($hls) {
										...
										$name = CHtml::link(CHtml::encode($cam->name), $this->createUrl('cams/hls').'?url=http://'.$server->ip.':'.$server->s_port.'/hls/'.$cam->id.'_low.m3u8', array('target' => '_blank'));
									}
						...
						<table class="table table-striped">
							...
							<tbody>
								...
								foreach ($mySharedCams as $key => $cam) {
									$name = CHtml::link(CHtml::encode($cam->cam->name), $this->createUrl('cams/archive', array('id' => $cam->cam->getSessionId(), 'full' => '1')), array('target' => '_blank'));
									if($hls) {
										if(!isset($sserver)) {
											$sserver = Servers::model()->findByPk($cam->cam->owner->server_id);
										}
										$name = CHtml::link(CHtml::encode($cam->cam->name), $this->createUrl('cams/hls').'?url=http://'.$sserver->ip.':'.$sserver->s_port.'/hls/'.$cam->cam->id.'_low.m3u8', array('target' => '_blank'));
									}
									echo '<tr>
						...
						<table class="table table-striped">
							...
							<tbody>
								...
								foreach ($myPublicCams as $key => $cam) {
									...
									if($hls) {
										if(!isset($pserver)) {
											$pserver = Servers::model()->findByPk($cam->owner->server_id);
										}
										$name = CHtml::link(CHtml::encode($cam->name), $this->createUrl('cams/hls').'?url=http://'.$pserver->ip.':'.$pserver->s_port.'/hls/'.$cam->id.'_low.m3u8', array('target' => '_blank'));
									}
									echo '<tr>
